<?php
/**
 * RecordingListTest
 *
 * PHP version 7.3
 *
 * @category Class
 * @package  FreeClimb\Api
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
 */

/**
 * FreeClimb API
 *
 * FreeClimb is a cloud-based application programming interface (API) that puts the power of the Vail platform in your hands. FreeClimb simplifies the process of creating applications that can use a full range of telephony features without requiring specialized or on-site telephony equipment. Using the FreeClimb REST API to write applications is easy! You have the option to use the language of your choice or hit the API directly. Your application can execute a command by issuing a RESTful request to the FreeClimb API. The base URL to send HTTP requests to the FreeClimb REST API is: /apiserver. FreeClimb authenticates and processes your request.
 *
 * The version of the OpenAPI document: 1.0.0
 * Contact: support@freeclimb.com
 * Generated by: https://openapi-generator.tech
 * OpenAPI Generator version: 5.4.0
 */

/**
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Please update the test case below to test the model.
 */

namespace FreeClimb\Api\Test\Model;

use FreeClimb\Api\Model\RecordingList;
use PHPUnit\Framework\TestCase;

/**
 * RecordingListTest Class Doc Comment
 *
 * @category    Class
 * @description RecordingList
 * @package     FreeClimb\Api
 * @author      OpenAPI Generator team
 * @link        https://openapi-generator.tech
 */
class RecordingListTest extends TestCase
{
    protected $RecordingList;

    /**
     * Test "RecordingList"
     */
    public function setUp() : void
    {
        $this->RecordingList = new RecordingList();
    }
    /**
     * Test attribute "total"
     */
    public function testPropertyTotal()
    {
        $this->RecordingList->setTotal(12345);
        $this->assertEquals($this->RecordingList->getTotal(), 12345);
        $this->assertIsInt($this->RecordingList->getTotal());
    }
    /**
     * Test attribute "start"
     */
    public function testPropertyStart()
    {
        $this->RecordingList->setStart(12345);
        $this->assertEquals($this->RecordingList->getStart(), 12345);
        $this->assertIsInt($this->RecordingList->getStart());
    }
    /**
     * Test attribute "end"
     */
    public function testPropertyEnd()
    {
        $this->RecordingList->setEnd(12345);
        $this->assertEquals($this->RecordingList->getEnd(), 12345);
        $this->assertIsInt($this->RecordingList->getEnd());
    }
    /**
     * Test attribute "page"
     */
    public function testPropertyPage()
    {
        $this->RecordingList->setPage(12345);
        $this->assertEquals($this->RecordingList->getPage(), 12345);
        $this->assertIsInt($this->RecordingList->getPage());
    }
    /**
     * Test attribute "num_pages"
     */
    public function testPropertyNumPages()
    {
        $this->RecordingList->setNumPages(12345);
        $this->assertEquals($this->RecordingList->getNumPages(), 12345);
        $this->assertIsInt($this->RecordingList->getNumPages());
    }
    /**
     * Test attribute "page_size"
     */
    public function testPropertyPageSize()
    {
        $this->RecordingList->setPageSize(12345);
        $this->assertEquals($this->RecordingList->getPageSize(), 12345);
        $this->assertIsInt($this->RecordingList->getPageSize());
    }
    /**
     * Test attribute "next_page_uri"
     */
    public function testPropertyNextPageUri()
    {
        $this->RecordingList->setNextPageUri('TEST_STRING');
        $this->assertStringContainsString($this->RecordingList->getNextPageUri(), 'TEST_STRING');
        $this->assertIsString($this->RecordingList->getNextPageUri());
        

    }
    /**
     * Test attribute "recordings"
     */
    public function testPropertyRecordings()
    {
        $this->RecordingList->setRecordings(array());
        $this->assertIsArray($this->RecordingList->getRecordings());
    }
}
